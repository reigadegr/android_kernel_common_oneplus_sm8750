...
static void crtl_update_refresh_rate(int pid, int tid, unsigned int vsyncNs)
{
	...
	if (is_sf) {
		...
		set_frame_group_window_size(DEFAULT_FRAME_GROUP_ID, vsyncNs);
#endif
		return;
	}
...
#else
	if (grp_id != DEFAULT_FRAME_GROUP_ID)
#endif
		return;
...
static long ofb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_VERBOSE))
		ofb_debug("CMD_ID[%d], stage[%d], pid[%d], tid[%d] vsyncNs[%lld]\n",
	...
	case CMD_ID_BOOST_HIT:
		...
		if (data.stage == BOOST_FRAME_START) {
			...
			set_frame_state(grp_id, FRAME_START);
		...
		if (data.stage == FRAME_BOOST_END) {
			...
			set_frame_state(grp_id, FRAME_END);
			default_group_update_cpufreq(grp_id);
		}
#endif
		...
		if (data.stage == BOOST_OBTAIN_VIEW) {
			if (get_frame_rate(grp_id) >= fbg_get_stune_boost(BOOST_UTIL_FRAME_RATE)) {
				set_frame_util_min(grp_id, fbg_get_stune_boost(BOOST_UTIL_MIN_OBTAIN_VIEW), true);
				default_group_update_cpufreq(grp_id);
			}
		}
		...
		if (data.stage == BOOST_FRAME_TIMEOUT) {
			if (get_frame_rate(grp_id) >= fbg_get_stune_boost(BOOST_UTIL_FRAME_RATE) && check_putil_over_thresh(grp_id, fbg_get_stune_boost(BOOST_UTIL_MIN_THRESHOLD))) {
				set_frame_util_min(grp_id, fbg_get_stune_boost(BOOST_UTIL_MIN_TIMEOUT), true);
				default_group_update_cpufreq(grp_id);
			}
		}
...
#endif
			inputmethod_set_boost_start(grp_id);
		}
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST_MULTI)
	set_frame_state(GAME_FRAME_GROUP_ID, FRAME_START);
#else
	set_frame_state(DEFAULT_FRAME_GROUP_ID, FRAME_START);
#endif
	...
	case CMD_ID_BOOST_HIT:
		...
		if (data.stage == BOOST_ADD_FRAME_TASK) {
			...
			if (add_rm_related_frame_task(grp_id, data.pid, data.tid, data.capacity_need,
				data.m_rtg.related_depth, data.related_width))
				ret = grp_id;
			else
				ret = 0;
		}
...
#else
		if (SYSTEM_UID == task_uid(current).val) {
