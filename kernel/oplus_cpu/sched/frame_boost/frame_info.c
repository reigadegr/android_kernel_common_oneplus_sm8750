...
void set_max_frame_rate(unsigned int frame_rate)
{
	...
	raw_spin_lock_irqsave(&default_frame_info.lock, flags);
	default_frame_info.frame_rate_max = frame_rate;
	__set_frame_rate(&default_frame_info, frame_rate);
	raw_spin_unlock_irqrestore(&default_frame_info.lock, flags);

	if (unlikely(sysctl_frame_boost_debug & DEBUG_KMSG))
		ofb_debug("set default frame group max frame rate[%d]\n", frame_rate);

	if (unlikely(sysctl_frame_boost_debug & DEBUG_FTRACE))
		trace_printk("frame_rate=%u frame_interval=%u vutil_time2max=%u\n",
			default_frame_info.frame_rate,
			default_frame_info.frame_interval,
			default_frame_info.vutil_time2max);
}
...
bool set_frame_rate(int grp_id, unsigned int frame_rate)
{
	...
	if ((frame_rate < MIN_FRAME_RATE) || (frame_rate > MAX_FRAME_RATE)) {
		ofb_err("invalid frame rate %d, min is %d, max is %d",
...
int set_frame_util_min(int grp_id, int min_util, bool clear)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		frame_min_util_systrace_c(grp_id, min_util);
	}
...
void set_frame_state(int grp_id, unsigned int state)
{
	...
	/* frame_min_util active time should be end with NextVsync */
	if (unlikely(frame_info->clear_limit && state == FRAME_START)) {
		...
		if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE))
			frame_min_util_systrace_c(grp_id, 0);
	}
	raw_spin_unlock_irqrestore(&frame_info->lock, flags);
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		if (state == FRAME_START)
			val_systrace_c(grp_id, 1, "frame_state");
		else if (state == FRAME_END)
			val_systrace_c(grp_id, 0, "frame_state");
	}
#endif
}
...
int frame_info_init(void)
{
	...
	ret = __frame_info_init(&default_frame_info);
	if (ret != 0)
		return ret;
