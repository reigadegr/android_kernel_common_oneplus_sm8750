		...
		if (*p != ' ' && *p != '\t' && *p != '\n') {
			...
			if (in_token) {
				...
				*p = '\0';
				...
				if (strncmp(token_start, "0x", 2) == 0 ||
				    strncmp(token_start, "0X", 2) == 0) {
					val = simple_strtoul(token_start,
							     &endptr, 16);
				} else {
					val = simple_strtoul(token_start,
							     &endptr, 10);
				...
				// 修改最大值检查，允许32位值
				if (endptr == token_start || *endptr != '\0' ||
				    val > 0xFFFFFFFFUL) {
					pr_err("overwriter: parse_numbers: invalid number '%s' (must be <= 0xFFFFFFFF)\n",
					       token_start);
					kfree(buf);
					kfree(dup);
					return -EINVAL;
				...
				buf[i * 4 + 0] = (u8)((val >> 24) &
						      0xFF); // Highest byte
				buf[i * 4 + 1] =
					(u8)((val >> 16) & 0xFF); // High byte
				buf[i * 4 + 2] =
					(u8)((val >> 8) & 0xFF); // Low byte
				buf[i * 4 + 3] =
					(u8)(val & 0xFF); // Lowest byte
...
static int __init remove_dt_node(const char *path)
{
	...
	if (!np) {
		pr_err("overwriter: %s: DT node not found: '%s'\n", PATCH_TAG,
		       path);
		return -ENODEV;
	}
	...
	if (!parent) {
		pr_err("overwriter: %s: Cannot remove root node: '%s'\n",
		       PATCH_TAG, path);
		of_node_put(np);
		return -EINVAL;
	}
...
static int __init remove_dt_property(const char *path, const char *prop_name)
{
	...
	if (!prop) {
		pr_err("overwriter: %s: Property '%s' not found in node '%s'\n",
		       PATCH_TAG, prop_name, path);
		ret = -EINVAL;
	}
...
static int __init create_dt_node(const char *path)
{
	...
	if (!node_name || node_name == path) {
		pr_err("overwriter: %s: Invalid path format for node creation: '%s'\n",
		       PATCH_TAG, path);
		return -EINVAL;
	}
	...
	if (!parent_path) {
		pr_err("overwriter: %s: Failed to allocate memory for parent path\n",
		       PATCH_TAG);
		return -ENOMEM;
	}
	...
	if (!parent) {
		pr_err("overwriter: %s: Parent node not found: '%s'\n",
	...
	if (!np) {
		pr_err("overwriter: %s: Failed to allocate memory for new node\n",
		       PATCH_TAG);
		of_node_put(parent);
		kfree(parent_path);
		return -ENOMEM;
	...
	if (!np->name) {
		pr_err("overwriter: %s: Failed to allocate memory for node name\n",
		       PATCH_TAG);
		kfree(np);
		of_node_put(parent);
		kfree(parent_path);
	...
	if (!np->full_name) {
		pr_err("overwriter: %s: Failed to allocate memory for full node name\n",
		       PATCH_TAG);
		kfree(np->name);
		kfree(np);
		of_node_put(parent);
...
static int parse_hex_bytes(const char *value_str, u8 **out_buf, size_t *out_len)
{
	...
	if (!value_str || strlen(value_str) < 2 || value_str[0] != '[' ||
	    value_str[strlen(value_str) - 1] != ']') {
		pr_err("overwriter: parse_hex_bytes: Invalid format, must be like '[00 FF 12]'\n");
		return -EINVAL;
	}
	...
	while (*p) {
		if (is_hex_digit(*p)) {
			...
			pr_err("overwriter: parse_hex_bytes: Invalid character '%c' in hex string '%s'\n",
			       *p, value_str);
			kfree(dup);
			return -EINVAL;
		}
	...
	while (*p && i < count) {
		...
		token_start = p;
		// 确保有两个十六进制字符
		if (!is_hex_digit(token_start[0]) ||
		    !is_hex_digit(token_start[1])) {
			pr_err("overwriter: parse_hex_bytes: Invalid hex byte format '%s' in '%s'\n",
		...
		if (endptr == byte_str || *endptr != '\0') {
			pr_err("overwriter: parse_hex_bytes: invalid hex byte '%s'\n",
			       byte_str);
			kfree(buf);
			kfree(dup);
			return -EINVAL;
...
static int __init create_dt_property(const char *path, const char *prop_name,
				     const char *value)
{
	struct device_node *np;
	...
	if (!prop) {
		pr_err("overwriter: %s: Failed to allocate memory for new property\n",
		       PATCH_TAG);
		of_node_put(np);
		return -ENOMEM;
	}
	...
	if (!prop->name) {
		pr_err("overwriter: %s: Failed to allocate memory for property name\n",
		       PATCH_TAG);
		kfree(prop);
		of_node_put(np);
		return -ENOMEM;
	...
	if (!value || value[0] == '\0') {
		...
		if (!bin_value) {
			pr_err("overwriter: %s: Failed to allocate memory for [00] value\n",
			       PATCH_TAG);
			kfree(prop->name);
			kfree(prop);
			of_node_put(np);
		...
		if (value[0] == '[' && value[strlen(value) - 1] == ']') {
			...
			if (ret != 0) {
				pr_err("overwriter: %s: Failed to parse hex byte value '%s': %d\n",
				       PATCH_TAG, value, ret);
				kfree(prop->name);
				kfree(prop);
				of_node_put(np);
			...
			if (is_string_value) {
				...
				if (!prop->value) {
					pr_err("overwriter: %s: Failed to allocate memory for string value\n",
					       PATCH_TAG);
					kfree(prop->name);
					kfree(prop);
					of_node_put(np);
...
static int __init patch_device_tree(const char *input)
{
	...
	if (!input || strlen(input) == 0) {
		pr_err("overwriter: %s: Invalid input\n", PATCH_TAG);
		return -EINVAL;
	}
	...
	if (!dup) {
		pr_err("overwriter: %s: Failed to duplicate input string\n",
		       PATCH_TAG);
		return -ENOMEM;
	}
	...
	operation = dup[0];
	if (operation == 'r' || operation == 'd' || operation == 'c' ||
	    operation == 'a') {
		...
		if (operation == 'r') {
			...
			if (!last_slash || last_slash == op_input) {
				pr_err("overwriter: %s: Invalid format for remove property operation: no valid path\n",
				       PATCH_TAG);
				kfree(dup);
				return -EINVAL;
			}
			...
			*last_slash = '\0';
			...
			if (!space_pos) {
				pr_err("overwriter: %s: Invalid format for create property operation: no value specified\n",
				       PATCH_TAG);
				kfree(dup);
				return -EINVAL;
			}
			...
			*space_pos = '\0';
			...
			if (*value == '\0') {
				pr_err("overwriter: %s: Empty value for create property operation\n",
				       PATCH_TAG);
				kfree(dup);
				return -EINVAL;
			}
			...
			if (!prop_name || prop_name == path) {
				pr_err("overwriter: %s: Invalid path format for create property: '%s'\n",
				       PATCH_TAG, path);
				kfree(dup);
				return -EINVAL;
			}
	...
	if (!space_pos) {
		pr_err("overwriter: %s: Invalid input format, no space found\n",
		       PATCH_TAG);
		kfree(dup);
		return -EINVAL;
	}
	...
	*space_pos = '\0';
	...
	if (*value == '\0') {
		pr_err("overwriter: %s: Empty value after path\n", PATCH_TAG);
		kfree(dup);
		return -EINVAL;
	}
	...
	prop_name = strrchr(path, '/');
	if (!prop_name || prop_name == path) {
		pr_err("overwriter: %s: Invalid path format: '%s'\n", PATCH_TAG,
		       path);
		kfree(dup);
		return -EINVAL;
	}
	...
	*prop_name = '\0';
	...
	if (value[0] == '[' && value[strlen(value) - 1] == ']') {
		...
		final_len =
			bin_len; // 对于直接写入字节，长度就是解析出来的字节长度

		prop->value = kzalloc(final_len, GFP_ATOMIC);
		if (!prop->value) {
			pr_err("overwriter: %s: Failed to allocate memory for hex byte value\n",
			       PATCH_TAG);
		...
		if (is_string_value) {
			...
			size_t str_len = strlen(value);
			size_t final_len =
				str_len + 1; /* Include null terminator */
			...
			prop->value = kzalloc(
				final_len,
				GFP_ATOMIC); /* kzalloc zeros the memory */
			if (!prop->value) {
				pr_err("overwriter: %s: Failed to allocate memory for numeric value\n",
				       PATCH_TAG);
				prop->value = old_value; /* Restore old value */
				kfree(bin_value);
				of_node_put(np);
			...
			if (bin_len > 0) {
				size_t hex_str_size =
					bin_len * 5 +
					1; /* "0xNN " per byte + null */
				char *hex_str =
					kmalloc(hex_str_size, GFP_ATOMIC);
				if (hex_str) {
					size_t j, pos = 0;
					hex_str[0] =
						'\0'; /* Initialize string */
...
static int __init overwrite_config_init(void)
{
	...
	for (int i = 0; i < overwrite_config_group_count; i++) {
		const struct overwrite_config_group *group =
			&overwrite_config_groups[i];

		if (strcmp(group->prefix, "common") == 0) {
			common_group = group;
		} else if (device_name &&
			   strcmp(group->prefix, device_name) == 0) {
			device_group = group;
		}
	}
